 ...
 * 模糊查询(LIKE) 条件注解
 * <p>
 * 支持对 String 类型字段按英文逗号分割后生成多个 OR LIKE 条件，
 * 当 splitByComma = true 时，会先去除字段值中的换行符，再按英文逗号拆分，每个元素单独生成 LIKE 条件并用 OR 连接。
 *
 * @author WcJun
 * @date 2019/05/12
 */
...
public @interface Like {
    ...
    /**
     * 是否将字符串字段按英文逗号分割为多个元素后生成多个 OR LIKE 条件
     * <p>
     * 当字段类型为 String 且该属性为 true 时，处理器会：
     * 1. 去除字段值中的换行符（\r、\n）
     * 2. 按英文逗号（,）分割字符串
     * 3. 过滤掉空白元素
     * 4. 每个元素单独生成 LIKE 条件（遵守 like() 指定的匹配模式），多个条件之间用 OR 连接
     *
