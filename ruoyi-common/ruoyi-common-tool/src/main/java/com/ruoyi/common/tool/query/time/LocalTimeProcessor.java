 ...
 * @date 2020/02/15
 */
public class LocalTimeProcessor implements TimeProcessor {
    @Override
    ...
    public boolean support(Class<?> timeType) {
        return LocalTime.class.isAssignableFrom(timeType);
    }
    ...
    public <T> T handleTime(Long timeStamp, Class<?> clazz) {
        return (T) java.time.LocalTime.ofInstant(java.time.Instant.ofEpochMilli(timeStamp), java.time.ZoneId.systemDefault());
    }
}