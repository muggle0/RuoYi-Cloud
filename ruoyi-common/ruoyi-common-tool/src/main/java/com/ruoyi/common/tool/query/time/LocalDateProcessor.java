 ...
 * @date 2020/02/15
 */
public class LocalDateProcessor implements TimeProcessor {
    @Override
    ...
    public boolean support(Class<?> timeType) {
        return LocalDate.class.isAssignableFrom(timeType);
    }
    ...
    public <T> T handleTime(Long timeStamp, Class<?> clazz) {
        return (T) java.time.LocalDate.ofInstant(java.time.Instant.ofEpochMilli(timeStamp), java.time.ZoneId.systemDefault());
    }
}